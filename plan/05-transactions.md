...
src/features/transactions/transactions.ts
```
...
- `useQuery({ queryKey: ['transactions', filters], ... })` against
  `/api/transactions`.
- Five columns: date · description (+ counterparty as a muted second line) ·
  category (badge) · method · amount (right-aligned, teal/coral, tabular
  figures). Plain `<table>` primitives — no table library.
